...
//        println(result.getResponse().getContentAsString());
    ...
    @Test
    @DirtiesContext
    fun `try to get single existent assignment`() {
        ...
        this.mvc.perform(
            get("/api/student/assignments/current/${assignmentId}")
                .contentType(MediaType.APPLICATION_JSON)
                .header("authorization", testsHelper.header("student1", token)))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.id", `is`("sampleJavaProject")))
            .andExpect(jsonPath("$.language", `is`("JAVA")))
            .andExpect(jsonPath("$.instructions.format", `is`("HTML")))
            .andExpect(jsonPath("$.instructions.body", containsString("<h2>Sample Java Assignment</h2>")))

    }

    @Test
    @DirtiesContext
    fun `try to get single nonexistent assignment`() {

        val token = generateToken("student1", mutableListOf(SimpleGrantedAuthority("ROLE_STUDENT")), mvc)

        this.mvc.perform(
            get("/api/student/assignments/current/nonexistentID")
                .contentType(MediaType.APPLICATION_JSON)
                .header("authorization", testsHelper.header("student1", token)))
