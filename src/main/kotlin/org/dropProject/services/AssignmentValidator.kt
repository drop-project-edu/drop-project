    ...
    private fun validateProperTestClasses(assignmentFolder: File, assignment: Assignment) {
        ...
        if (testClasses.isEmpty()) {
            ...
            for (testClass in testClasses) {
                ...
                invalidTestMethods += content.filter { it.contains("@Test()") }.count()
            ...
            if (validTestMethods == 0) {
                report.add(Info(InfoType.WARNING, "You haven't defined any test methods.", "Use the @Test annotation to mark test methods."))
            } else {
                report.add(Info(InfoType.INFO, "You have defined ${validTestMethods} test methods."))
            }
        }
