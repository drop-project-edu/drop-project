...
class StudentAPIController(
    ...
    @RequestMapping(value = ["/assignments/current/{assignmentID}"], method = [(RequestMethod.GET)],  produces = [MediaType.APPLICATION_JSON_VALUE])
    @JsonView(JSONViews.StudentAPI::class)  // to publish only certain fields of the Assignment
    @ApiOperation(value = "Get specific assignment details")
    fun getSingleAssignment(principal: Principal, @PathVariable assignmentID: String): ResponseEntity<Assignment> {
        ...
        return if (assignment.error != null) {
            ResponseEntity.internalServerError().body(assignment)
        } else {
            //control access on w-list
            assignmentService.checkAssignees(assignmentID,principal.realName())
            ResponseEntity.ok().body(assignment)
        }
